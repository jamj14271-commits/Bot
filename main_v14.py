...
async def menu(ctx):
    ...
    embed.add_field(name="🏁 Challenge", value="`!nopchallenge [ID level] [tên]` nộp challenge kèm bằng chứng.\n`!bxhchallenge` xem BXH Challenge.", inline=True)
...
async def nopchallenge(ctx, level_id: str = None, *, ten: str = None):
    ...
    if not level_id or not ten:
        return await ctx.send("❌ VD: `!nopchallenge 12345678 Tên Level/Challenge` (kèm ảnh/video bằng chứng)")
