...
async def on_ready():
    ...
    if not _views_registered:
        ...
        print("✅ Đã đăng ký persistent views (ReviewView, ReportReviewView).")
...
async def menu(ctx):
    ...
    embed.add_field(name="🏁 Challenge", value="`!nopchallenge [tên]` nộp challenge kèm bằng chứng.\n`!bxhchallenge` xem BXH Challenge.", inline=True)
    
    if is_admin:
        embed.add_field(name="🛠️ LỆNH ADMIN", value="• `!setmp @user [số]` đặt lại điểm\n• `!addmp @user [số]` cộng/trừ điểm\n• `!thongbao [event/daily] [ID]` cập nhật mục tiêu\n• `!dondanhieu` dọn dữ liệu active_title bị lỗi\n• `!ratechallenge [id] [lý do nếu từ chối]` duyệt/từ chối Challenge\n• `!bxhchallenge [id] [vị trí]` xếp hạng Challenge", inline=False)
...
# ================= HỆ THỐNG CHALLENGE (MỚI - v13) ================= #
# ĐÃ THÊM MỚI: hệ thống Challenge tách riêng khỏi MP/danh hiệu thường. Member tự nộp
# challenge (level tự chọn/tự thử thách) kèm bằng chứng, Admin xem xét và dùng lệnh
# !ratechallenge để duyệt/từ chối. Khác với !duyet, việc XẾP HẠNG trên BXH Challenge
# KHÔNG tự động - Admin toàn quyền xếp/xếp lại vị trí bất kỳ lúc nào qua !bxhchallenge,
# vì độ khó/chất lượng challenge cần đánh giá thủ công, không thể so sánh bằng số như MP.
@bot.command(aliases=["submitchallenge", "nopchall"])
@require_db()
async def nopchallenge(ctx, *, ten: str = None):
    if not ten: return await ctx.send("❌ VD: `!nopchallenge Tên Level/Challenge` (kèm ảnh/video bằng chứng)")
    if not ctx.message.attachments: return await ctx.send("📸 Phải gửi kèm bằng chứng (ảnh/video) nhé!")
    ...
    if admin_channel:
        ...
        embed.add_field(name="👤 Người nộp", value=ctx.author.mention, inline=True)
        embed.add_field(name="🏷️ Tên Challenge", value=f"**{ten}**", inline=False)
        embed.add_field(name="🔗 Bằng chứng", value=f"[Xem tại đây]({attachment_url})", inline=False)
        embed.set_image(url=attachment_url)
        embed.set_footer(text=f"Dùng !ratechallenge {challenge_id} để duyệt, hoặc !ratechallenge {challenge_id} [lý do] để từ chối")
        await admin_channel.send(embed=embed)
    await ctx.send(f"✅ Đã nộp challenge **{ten}** (Mã `#{challenge_id}`) cho Admin xem xét!")

@bot.command()
@commands.has_permissions(administrator=True)
@require_db()
async def ratechallenge(ctx, challenge_id: int, *, ly_do_tu_choi: str = None):
    challenge = await db.challenges.find_one({"_id": challenge_id})
    if not challenge: return await ctx.send(f"❌ Không tìm thấy challenge có mã `#{challenge_id}`.")
    if challenge["status"] != "pending":
        return await ctx.send(f"⚠️ Challenge `#{challenge_id}` đã được xử lý trước đó (trạng thái hiện tại: **{challenge['status']}**).")

    submitter = await bot.fetch_user(challenge["submitter_id"])

    if ly_do_tu_choi:
        await db.challenges.update_one({"_id": challenge_id}, {"$set": {"status": "rejected"}})
        await log_history(challenge["submitter_id"], "challenge", challenge["name"], "rejected", ctx.author.id, ly_do_tu_choi)
        await log_admin_action(ctx.author.mention, f"Từ chối Challenge #{challenge_id}: **{challenge['name']}**", f"<@{challenge['submitter_id']}>", ly_do_tu_choi)
        if submitter:
            try: await submitter.send(f"❌ Challenge **{challenge['name']}** (Mã `#{challenge_id}`) của bạn bị từ chối.\n**Lý do:** {ly_do_tu_choi}")
            except discord.Forbidden: pass
        return await ctx.send(f"❌ Đã từ chối challenge `#{challenge_id}` — **{challenge['name']}**.")

    # ĐÃ THÊM MỚI: duyệt xong KHÔNG tự gán hạng - rank để None, chờ Admin tự xếp bằng
    # !bxhchallenge để đảm bảo việc xếp hạng luôn do con người đánh giá, không tự động.
    await db.challenges.update_one({"_id": challenge_id}, {"$set": {"status": "approved"}})
    await log_history(challenge["submitter_id"], "challenge", challenge["name"], "approved", ctx.author.id, "Đã duyệt, chờ Admin xếp hạng")
    await log_admin_action(ctx.author.mention, f"Duyệt Challenge #{challenge_id}: **{challenge['name']}**", f"<@{challenge['submitter_id']}>", "Chờ xếp hạng")
    if submitter:
        try: await submitter.send(f"🎉 Challenge **{challenge['name']}** (Mã `#{challenge_id}`) của bạn đã được duyệt! Admin sẽ sớm xếp hạng cho bạn trên BXH Challenge.")
        except discord.Forbidden: pass
    await ctx.send(f"✅ Đã duyệt challenge `#{challenge_id}` — **{challenge['name']}**.\n💡 Dùng `!bxhchallenge {challenge_id} <vị_trí>` để xếp hạng cho challenge này.")

@bot.command(aliases=["bxhchall", "challengeboard"])
@require_db()
async def bxhchallenge(ctx, challenge_id: int = None, vi_tri: int = None):
    # Có đủ 2 tham số -> đây là thao tác XẾP HẠNG, chỉ Admin được phép.
    if challenge_id is not None and vi_tri is not None:
        ...
        if challenge["status"] != "approved":
            return await ctx.send(f"❌ Challenge `#{challenge_id}` chưa được duyệt (dùng `!ratechallenge {challenge_id}` trước).")
        ...
        await log_admin_action(ctx.author.mention, f"Xếp hạng Challenge #{challenge_id}: **{challenge['name']}**", f"<@{challenge['submitter_id']}>", f"Vị trí mới: #{vi_tri}")
        return await ctx.send(f"✅ Đã xếp challenge `#{challenge_id}` — **{challenge['name']}** vào vị trí **#{vi_tri}** trên BXH Challenge.")
    ...
    if ranked:
        dong = [f"**#{c['rank']}** — {c['name']} (<@{c['submitter_id']}>) `#{c['_id']}`" for c in ranked]
        embed.add_field(name="🏆 Đã xếp hạng", value="\n".join(dong)[:1024], inline=False)
    if unranked:
        dong2 = [f"{c['name']} (<@{c['submitter_id']}>) `#{c['_id']}`" for c in unranked]
    ...
    embed.set_footer(text="Nộp challenge: !nopchallenge [tên] | Admin xếp hạng: !bxhchallenge [id] [vị trí]")
    await ctx.send(embed=embed)
# ==================================================================== #
...
async def history(ctx, member: discord.Member = None):
    ...
    for log in logs:
        icon = "✅" if log["status"] == "approved" else "❌"
