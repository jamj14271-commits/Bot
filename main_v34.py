...
# ĐÃ FIX: trước đây nếu quên đổi ADMIN_LOG_CHANNEL_ID (vẫn để 0) thì bot chạy bình thường
# nhưng log_admin_action() sẽ luôn thất bại ÂM THẦM (bot.get_channel(0) trả về None), rất
# khó phát hiện. Giờ cảnh báo rõ ngay lúc khởi động, giống cách cảnh báo MONGO_URI.
if not ADMIN_LOG_CHANNEL_ID:
    print("⚠️ CẢNH BÁO: Chưa cấu hình ADMIN_LOG_CHANNEL_ID (đang để 0)! Log hành động admin sẽ không được gửi cho đến khi cập nhật ID kênh thật.")
...
# ================= THÊM MỚI: HỆ THỐNG CP (CHALLENGE POINT) & CCP (CREATOR CHALLENGE POINT) ================= #
# CP: điểm riêng cho việc VƯỢT QUA (beat) 1 challenge đã có trên BXH Challenge, tách biệt
# hoàn toàn với MP. Mọi challenge đều CỐ ĐỊNH 500 CP khi được ghi nhận "beat" (qua !duyet
# [mã/tên challenge] kèm bằng chứng), KHÔNG phân biệt độ khó - vì độ khó challenge không
# có thang đo chuẩn như DIFFICULTY_MP.
# Nếu người được ghi nhận là "người Verify" (tag @user ở trường Verify lúc !nopchallenge),
# họ tự động nhận thêm CP_VERIFY_BONUS (250) CỘNG với CP_PER_BEAT (500) = 750 CP, vì Verify
# nghĩa là họ cũng đã tự beat challenge đó để xác nhận nó khả thi.
# CCP: điểm riêng cho người TẠO level (tag @user ở trường "người tạo" lúc !nopchallenge),
# ghi nhận công sức thiết kế level chứ không phải công sức vượt qua nó.
CP_PER_BEAT = 500
CP_VERIFY_BONUS = 250
CCP_PER_CREATE = 250

# ĐÃ THÊM MỚI: chống spam gửi yêu cầu duyệt "Beat Challenge" liên tục (tránh 1 người gõ
# !duyet [mã] hàng loạt làm ngập kênh admin). Cooldown lưu TRONG BỘ NHỚ (mất khi bot
# restart) - chỉ để chặn spam ngắn hạn, không cần bền vững qua restart.
_beat_challenge_cooldown = {}  # user_id -> thời điểm gửi gần nhất (datetime)
BEAT_CHALLENGE_COOLDOWN_SECONDS = 30

# ĐÃ THÊM MỚI: dò tìm @mention Discord (dạng <@123...> hoặc <@!123...>) trong 1 đoạn text
# tự do (VD trường "tên người Verify"/"tên người tạo" của !nopchallenge), để tự động cộng
# CP/CCP cho đúng người được tag mà KHÔNG cần thêm cú pháp lệnh riêng - chỉ cần gõ @ rồi
# chọn tên trên bàn phím Discord như bình thường khi nộp challenge.
MENTION_REGEX = re.compile(r"<@!?(\d+)>")

def extract_user_id(text: str):
    """Trả về user ID (int) nếu text có chứa 1 @mention Discord thật, ngược lại None
    (VD người dùng chỉ gõ tên chữ thường như "Nguyễn Văn A" không phải mention)."""
...
TITLE_COLORS = {
    ...
    # --- J. Challenge/CP/CCP (tự động nhận, KHÔNG qua !duyet tay - xem TITLES_CHALLENGE) ---
    "thử thách đầu tiên": "37", "thử thách mới": "36",
    "huyền thoại challenge": "34", "vua challenge": "31",
    "thần challenge": "35", "bậc thầy verify": "35",
    "người mở đường": "32", "kiến trúc sư challenge": "32",
    "song kiếm challenge": "33", "vô song cp": "31",
    "vua verify challenge": "31",
}
...
# ĐÃ THÊM MỚI: đánh số phiên bản NGAY TRONG FILE, để lúc test/debug biết chắc bot đang
# chạy đúng bản nào (tránh lẫn lộn giữa các lần sửa liên tiếp như đợt debug CP/CCP vừa
# rồi). Mỗi lần chỉnh sửa file thì tăng số này lên + cập nhật CHANGELOG bên dưới.
# Xem !version để bot tự báo số này ngay trong Discord, không cần mở file ra đọc.
BOT_VERSION = "v34"
BOT_CHANGELOG = "Đồng bộ thông báo công khai cho TẤT CẢ danh hiệu (không chỉ Vua Cày Điểm/Vô Song CP/Challenge/adddanhhieu): thêm cho Vua Hardest, Vua Try Hard, Vua Verify Challenge, 5 danh hiệu tier Demon (newbie/sự khởi đầu/pro/hardcore player/huyền thoại), và mọi danh hiệu duyệt qua !duyet."
...
async def on_ready():
    ...
    if not _views_registered:
        ...
        bot.add_view(ChallengeBeatReviewView())
        _views_registered = True
        print("✅ Đã đăng ký persistent views (ReviewView, ReportReviewView, ChallengeReviewView, ChallengeBeatReviewView).")
...
async def check_and_transfer_top1_mp(guild):
    ...
    # ĐÃ FIX: nguyên tử hoá đọc+ghi chủ sở hữu cũ bằng find_one_and_update. Trước đây đọc
    # bằng find_one rồi ghi bằng update_one riêng lẻ có thể tạo race condition nếu hàm này
    # được gọi gần như đồng thời (VD 2 lượt cộng MP xử lý cùng lúc), khiến 2 lệnh gọi cùng
    # đọc một old_top1_id trước khi bên nào kịp ghi, dẫn tới tước/gán nhầm.
    config = await db.settings.find_one_and_update(
        {"_id": "top1_mp_owner"},
        {"$set": {"user_id": new_top1_id}},
        upsert=True,
...
# ================= THÊM MỚI: HÀM DÙNG CHUNG CHO CP/CCP ================= #
async def add_user_cp(user_id, amount, guild=None):
    await db.users.update_one({"_id": user_id}, {"$inc": {"cp": amount}}, upsert=True)
    if guild:
        await check_and_transfer_top1_cp(guild)

async def add_user_ccp(user_id, amount):
    await db.users.update_one({"_id": user_id}, {"$inc": {"ccp": amount}}, upsert=True)

# ĐÃ THÊM MỚI: giống hệt check_and_transfer_top1_mp nhưng cho CP - Top 1 CP toàn server
# tự động nhận danh hiệu "vô song cp", KHÔNG thể duyệt tay qua !duyet.
async def check_and_transfer_top1_cp(guild):
    users = await db.users.find({"cp": {"$gt": 0}}).sort("cp", -1).limit(1).to_list(1)
    if not users: return
    new_top1_id = users[0]["_id"]

    # ĐÃ FIX: nguyên tử hoá đọc+ghi chủ sở hữu cũ (xem giải thích ở check_and_transfer_top1_mp).
    config = await db.settings.find_one_and_update(
        {"_id": "top1_cp_owner"},
        {"$set": {"user_id": new_top1_id}},
        upsert=True,
    ...
    if old_top1_id != new_top1_id:
        if old_top1_id:
            await revoke_title(old_top1_id, "vô song cp")
        await db.users.update_one({"_id": new_top1_id}, {"$addToSet": {"titles": "vô song cp"}, "$set": {"active_title": "vô song cp"}}, upsert=True)

        thong_bao_channel = bot.get_channel(WELCOME_CHANNEL_ID)
        if thong_bao_channel:
            embed_top1 = discord.Embed(
                title="🏆 VÔ SONG CP MỚI!",
                description=f"<@{new_top1_id}> vừa vươn lên **Top 1 CP** (Challenge Point) toàn server!",
                color=discord.Color.gold()
            )
            try:
                await thong_bao_channel.send(embed=embed_top1)
...
# ĐÃ THÊM MỚI: 8 trong 10 danh hiệu Challenge dựa trên số lượng challenge người dùng đã
# BEAT/VERIFY/TẠO (lưu trong beaten_challenges/verified_challenges/created_challenges của
# db.users, mỗi mảng chỉ chứa ID challenge DUY NHẤT nhờ $addToSet, nên không thể lách luật
# bằng cách nộp/verify đi verify lại CÙNG 1 challenge). 2 danh hiệu còn lại ("vô song cp" -
# top 1 CP, xử lý ở check_and_transfer_top1_cp) không nằm trong bảng này vì cơ chế khác hẳn
# (độc quyền 1 người, có thể bị thu hồi khi mất top1) thay vì cứ đạt mốc là giữ vĩnh viễn.
CHALLENGE_TITLE_RULES = [
    ("beaten", 1, "thử thách đầu tiên"),
    ("beaten", 10, "thử thách mới"),
    ("beaten", 50, "huyền thoại challenge"),
    ("beaten", 100, "vua challenge"),
    ("verified", 5, "thần challenge"),
    ("verified", 15, "bậc thầy verify"),
    ("created", 5, "người mở đường"),
    ("created", 20, "kiến trúc sư challenge"),
]

async def check_and_grant_challenge_titles(user_id: int):
    """Tự động cấp danh hiệu liên quan Challenge dựa trên số lượng challenge đã
    BEAT/VERIFY/TẠO của người dùng. Gọi lại hàm này mỗi khi 1 trong 3 số liệu trên thay
    đổi (sau khi Admin duyệt !duyet beat challenge, hoặc sau khi challenge có tag Verify/
    người tạo được duyệt lần đầu). Trả về danh sách TÊN danh hiệu MỚI vừa mở khoá trong
    lần gọi này (rỗng nếu không có gì mới), để báo cho người dùng biết."""
    user = await db.users.find_one({"_id": user_id}) or {}
    so_beat = len(user.get("beaten_challenges", []))
    so_verify = len(user.get("verified_challenges", []))
    so_tao = len(user.get("created_challenges", []))
    da_co = set(user.get("titles", []))

    danh_hieu_moi = []
    so_luong_theo_loai = {"beaten": so_beat, "verified": so_verify, "created": so_tao}
    for loai, nguong, ten_danh_hieu in CHALLENGE_TITLE_RULES:
        if so_luong_theo_loai[loai] >= nguong and ten_danh_hieu not in da_co:
            danh_hieu_moi.append(ten_danh_hieu)

    # Danh hiệu kết hợp: vừa vượt qua >=5 challenge, vừa được Verify >=5 challenge.
    if so_beat >= 5 and so_verify >= 5 and "song kiếm challenge" not in da_co:
        danh_hieu_moi.append("song kiếm challenge")

    if danh_hieu_moi:
        await db.users.update_one(
            {"_id": user_id},
            {"$addToSet": {"titles": {"$each": danh_hieu_moi}}, "$set": {"active_title": danh_hieu_moi[-1]}},
            upsert=True
        )
        # ĐÃ THÊM MỚI: thông báo công khai ở kênh WELCOME_CHANNEL_ID mỗi khi ai đó mở khoá
        # 1 trong 10 danh hiệu Challenge/CP/CCP, giống cách server thông báo Vua Cày Điểm
        # mới - để cả server biết và ăn mừng cùng, không chỉ nhắn riêng người đó.
        thong_bao_channel = bot.get_channel(WELCOME_CHANNEL_ID)
        if thong_bao_channel:
            for ten_dh in danh_hieu_moi:
                embed_tb = discord.Embed(
                    title="🎖️ DANH HIỆU CHALLENGE MỚI!",
                    description=f"<@{user_id}> vừa mở khoá danh hiệu **{ten_dh.title()}**!",
                    color=discord.Color.gold()
                )
                try:
                    await thong_bao_channel.send(embed=embed_tb)
                except (discord.Forbidden, discord.HTTPException):
                    pass
    return danh_hieu_moi
# ================================================================================ #

# ĐÃ THÊM MỚI: 1 hàm DÙNG CHUNG để thông báo công khai ở kênh welcome mỗi khi có danh
# hiệu "độc quyền top 1" mới (Vua Cày Điểm, Vô Song CP, Vua Hardest, Vua Try Hard, Vua
# Verify Challenge...) - trước đây mỗi nơi tự viết 1 đoạn embed riêng, dễ bị quên/thiếu
# nhất quán (VD Vua Hardest/Try Hard/Verify Challenge từng KHÔNG có thông báo trong khi
# Vua Cày Điểm/Vô Song CP có). Gom về 1 hàm để thêm danh hiệu mới sau này không bị quên.
async def _thong_bao_danh_hieu_cong_khai(user_id: int, ten_danh_hieu: str, tieu_de: str):
    thong_bao_channel = bot.get_channel(WELCOME_CHANNEL_ID)
    if not thong_bao_channel: return
    embed = discord.Embed(
        title=tieu_de,
        description=f"<@{user_id}> vừa trở thành **{ten_danh_hieu.title()}** mới của server!",
        color=discord.Color.gold()
    )
    try:
        await thong_bao_channel.send(embed=embed)
    except (discord.Forbidden, discord.HTTPException):
        pass

async def grant_vua_hardest(guild, new_owner_id):
    # ĐÃ FIX: dùng find_one_and_update (nguyên tử) thay vì find_one rồi update_one riêng lẻ.
    # Trước đây nếu 2 admin bấm Duyệt gần như cùng lúc cho 2 người khác nhau, cả hai request
    # đều có thể đọc CÙNG 1 old_owner_id trước khi bên nào kịp ghi, dẫn tới tước/gán nhầm
    # người giữ danh hiệu độc quyền. Nguyên tử hoá bước đọc+ghi sẽ loại bỏ khoảng hở này.
    config = await db.settings.find_one_and_update(
        {"_id": "vua_hardest_owner"},
    ...
    # ĐÃ THÊM MỚI: đồng bộ với Vua Cày Điểm/Vô Song CP - MỌI danh hiệu "độc quyền top 1"
    # đều phải thông báo công khai như nhau, tránh cảm giác "cấp xong không thấy gì" mà
    # người dùng từng phản ánh với !adddanhhieu.
    await _thong_bao_danh_hieu_cong_khai(new_owner_id, "vua hardest", "🏆 VUA HARDEST MỚI!")

async def grant_vua_try_hard(guild, new_owner_id):
    # ĐÃ FIX: nguyên tử hoá đọc+ghi chủ sở hữu cũ, tránh race condition khi 2 admin duyệt
    # gần như cùng lúc (xem giải thích chi tiết ở grant_vua_hardest).
    config = await db.settings.find_one_and_update(
        {"_id": "vua_try_hard_owner"},
        {"$set": {"user_id": new_owner_id}},
        upsert=True,
        return_document=ReturnDocument.BEFORE,
    )
    ...
    await _thong_bao_danh_hieu_cong_khai(new_owner_id, "vua try hard", "🔥 VUA TRY HARD MỚI!")

# ĐÃ THÊM MỚI: danh hiệu "Vua Verify Challenge" - độc quyền 1 người (giống Vua Hardest/Vua
# Try Hard), tự động cấp khi 1 người Verify liên tiếp ≥5 challenge với ĐỘ KHÓ TĂNG DẦN (hạng
# challenge sau phải NHỎ HƠN hạng challenge trước, vì rank #1 = khó nhất trên BXH Challenge).
# Streak được tính ở ChallengeRankModal.on_submit (xem verify_streak/verify_last_rank).
async def grant_vua_verify_challenge(guild, new_owner_id):
    # ĐÃ FIX: nguyên tử hoá đọc+ghi chủ sở hữu cũ, tránh race condition khi 2 admin duyệt
    # gần như cùng lúc (xem giải thích chi tiết ở grant_vua_hardest).
    config = await db.settings.find_one_and_update(
        {"_id": "vua_verify_challenge_owner"},
        {"$set": {"user_id": new_owner_id}},
        upsert=True,
        return_document=ReturnDocument.BEFORE,
    )
    ...
    if old_owner_id and old_owner_id != new_owner_id:
        await revoke_title(old_owner_id, "vua verify challenge")
    await db.users.update_one({"_id": new_owner_id}, {"$addToSet": {"titles": "vua verify challenge"}, "$set": {"active_title": "vua verify challenge"}}, upsert=True)
    await _thong_bao_danh_hieu_cong_khai(new_owner_id, "vua verify challenge", "👑 VUA VERIFY CHALLENGE MỚI!")
...
YOUTUBE_URL_REGEX = re.compile(
    r"(?:https?://)?(?:www\.)?(?:m\.)?(?:youtube\.com/(?:watch\?v=|shorts/|live/)|youtu\.be/)([a-zA-Z0-9_-]{11})(?:[?&][^\s]*)?"
)
...
async def find_level_by_name(name: str):
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            # ĐÃ FIX: tên level đọc ra từ Gemini có thể chứa dấu cách, dấu tiếng Việt, hoặc
            # ký tự đặc biệt (&, #, ?...). Trước đây ghép thẳng vào URL bằng f-string khiến
            # request tới GDBrowser bị cắt cụt hoặc lỗi/không khớp kết quả. Giờ encode đúng
            # chuẩn URL trước khi gửi.
            encoded_name = urllib.parse.quote(name, safe="")
            url = f"https://gdbrowser.com/api/search/{encoded_name}?count=1"
            async with session.get(url) as r:
    ...
    async def approve_btn(self, interaction: discord.Interaction, button: Button):
        ...
        if req_type == "mp":
            ...
            if auto_title:
                ...
                # ĐÃ THÊM MỚI: đồng bộ thông báo công khai - trước đây các danh hiệu tier Demon
                # (newbie/sự khởi đầu/pro/hardcore player/huyền thoại) chỉ DM riêng, không đăng
                # kênh welcome như 10 danh hiệu Challenge, gây thiếu nhất quán.
                thong_bao_channel_dh = bot.get_channel(WELCOME_CHANNEL_ID)
                if thong_bao_channel_dh:
                    embed_dh = discord.Embed(title="🎖️ DANH HIỆU MỚI!", description=f"<@{user_id}> vừa mở khoá danh hiệu **{auto_title.title()}**!", color=discord.Color.gold())
                    try:
                        await thong_bao_channel_dh.send(embed=embed_dh)
                    except (discord.Forbidden, discord.HTTPException):
                        pass
            ...
            if role_name == "vua hardest":
                ...
                # ĐÃ THÊM MỚI: đồng bộ thông báo công khai cho MỌI danh hiệu duyệt qua !duyet,
                # giống hệt !adddanhhieu/10 danh hiệu Challenge/các ngôi vị top 1.
                thong_bao_channel_role = bot.get_channel(WELCOME_CHANNEL_ID)
                if thong_bao_channel_role:
                    embed_role = discord.Embed(title="🎖️ DANH HIỆU MỚI!", description=f"<@{user_id}> vừa nhận danh hiệu **{role_name.title()}**!", color=discord.Color.gold())
                    try:
                        await thong_bao_channel_role.send(embed=embed_role)
                    except (discord.Forbidden, discord.HTTPException):
                        pass
...
# ================= CÁC LỆNH BOT ================= #
# ĐÃ THÊM MỚI: !version - báo số phiên bản đang chạy + thay đổi gần nhất, để test/debug
# không cần đoán xem bot đã deploy đúng bản mới hay chưa (xem BOT_VERSION ở đầu file).
@bot.command(aliases=["ver", "phienban"])
async def version(ctx):
    embed = discord.Embed(title=f"🤖 Robtop Shoppe — Phiên bản {BOT_VERSION}", description=BOT_CHANGELOG, color=discord.Color.blurple())
...
async def menu(ctx):
    ...
    embed.add_field(name="🏁 Challenge & CP/CCP", value="`!nopchallenge [ID level] [tên] | [tên Verify] | [tên người tạo]` nộp challenge MỚI kèm bằng chứng (tag @ ở Verify/người tạo để họ tự động nhận CP/CCP).\n`!duyet [mã/tên challenge]` kèm bằng chứng để xin ghi nhận **đã BEAT** 1 challenge có sẵn trên BXH (+500 CP).\n`!bxhchallenge` xem BXH Challenge · `!bxhcp` xem BXH CP · `!bxhccp` xem BXH CCP · `!bxhtong` xem BXH uy tín tổng.\n`!challengeinfo [mã]` xem chi tiết 1 challenge.\n`!editbxhchallenge [id] [tên]|[Verify]|[người tạo]` (Admin) sửa thông tin challenge đã nộp.\n`!profile` xem CP/CCP và challenge khó nhất bạn đã vượt qua.", inline=True)
    
    if is_admin:
        embed.add_field(name="🛠️ LỆNH ADMIN", value="• `!setmp @user [số]` đặt lại điểm MP\n• `!addmp @user [số]` cộng/trừ MP\n• `!setcp/!addcp @user [số]` đặt lại/cộng trừ CP\n• `!setccp/!addccp @user [số]` đặt lại/cộng trừ CCP\n• `!adddanhhieu @user [danh hiệu] | [tên hardest nếu có]` cấp danh hiệu thủ công\n• `!xoadanhhieu @user [danh hiệu]` gỡ danh hiệu thủ công\n• `!themluotbeat @user [mã challenge]` ghi nhận thủ công 1 lượt beat (+500 CP)\n• `!xoaluotbeat @user [mã challenge]` gỡ 1 lượt beat đã ghi nhận (-500 CP)\n• `!thongbao [event/daily] [ID - có thể bỏ trống để bot tự chọn ngẫu nhiên]`\n📌 Daily/Event cũng TỰ ĐỘNG re-roll level mới ngay khi hết hạn, không cần Admin can thiệp.\n• `!dondanhieu` dọn dữ liệu active_title bị lỗi\n• Challenge mới: bấm nút Duyệt/Từ chối trên tin nhắn ở kênh admin\n• Beat Challenge (`!duyet [mã/tên]`): bấm nút Duyệt/Từ chối trên tin nhắn ở kênh admin\n• `!bxhchallenge [id] [vị trí]` xếp lại hạng Challenge\n• `!xoachallenge [id] [lý do]` gỡ challenge gian lận (hack/ăn cắp)", inline=False)
        
    embed.set_footer(text=f"💡 Nhớ gửi kèm ảnh/video khi dùng !duyet hay !report nhé! | Phiên bản {BOT_VERSION} (xem !version)")
    await ctx.send(embed=embed)
...
async def duyet(ctx, *, yeu_cau: str = None):
    if not yeu_cau: return await ctx.send("❌ VD: `!duyet easy demon`, `!duyet vua hardest`, hoặc `!duyet [mã/tên challenge]` để xin ghi nhận đã vượt qua 1 challenge trên BXH Challenge")
    ...
    if yeu_cau in ("vô song cp", "vua verify challenge") or yeu_cau in [t for _, _, t in CHALLENGE_TITLE_RULES] or yeu_cau == "song kiếm challenge":
        return await ctx.send(f"❌ Danh hiệu **{yeu_cau.title()}** được cấp TỰ ĐỘNG dựa trên hoạt động Challenge của bạn, không thể duyệt thủ công!")
    ...
    if not is_mp and not is_role:
        # ĐÃ THÊM MỚI: chống spam - chặn gửi yêu cầu Beat Challenge liên tục trong
        # BEAT_CHALLENGE_COOLDOWN_SECONDS giây (xem giải thích ở khai báo hằng số).
        now_ts = datetime.now(VN_TZ)
        last_ts = _beat_challenge_cooldown.get(ctx.author.id)
        if last_ts and (now_ts - last_ts).total_seconds() < BEAT_CHALLENGE_COOLDOWN_SECONDS:
            con_lai = BEAT_CHALLENGE_COOLDOWN_SECONDS - (now_ts - last_ts).total_seconds()
            return await ctx.send(f"⏳ Bạn vừa gửi yêu cầu duyệt Beat Challenge, vui lòng đợi thêm **{con_lai:.0f} giây** rồi thử lại!")

        # ĐÃ THÊM MỚI: !duyet [mã hoặc tên challenge] - dùng để xin ghi nhận đã VƯỢT QUA
        # (beat) 1 challenge ĐÃ CÓ SẴN trên BXH Challenge (khác với !nopchallenge - đề xuất
        # challenge MỚI). Tra theo MÃ (số) trước, nếu không phải số thì tra theo TÊN (không
        # phân biệt hoa/thường). Nếu trùng tên nhiều challenge, báo dùng mã thay vì tên.
        raw_id = yeu_cau.lstrip("#")
        if raw_id.isdigit():
            challenge_match = await db.challenges.find_one({"_id": int(raw_id), "status": "approved"})
            if not challenge_match:
                return await ctx.send(f"❌ Không tìm thấy challenge đã duyệt với mã `#{raw_id}` trên BXH Challenge.")
        else:
            ung_vien = await db.challenges.find({"status": "approved"}).to_list(None)
            trung_ten = [c for c in ung_vien if (c.get("name") or "").strip().lower() == yeu_cau.strip().lower()]
            if not trung_ten:
                return await ctx.send("❌ Yêu cầu không hợp lệ! (không phải độ khó/danh hiệu, cũng không khớp challenge nào đã duyệt trên BXH Challenge)")
            if len(trung_ten) > 1:
                danh_sach_ma = ", ".join(f"`#{c['_id']}`" for c in trung_ten)
                return await ctx.send(
                    f"⚠️ Có **{len(trung_ten)} challenge trùng tên** \"{yeu_cau}\" trên BXH! "
                    f"Vui lòng dùng **mã challenge** thay vì tên, VD: `!duyet {trung_ten[0]['_id']}`.\n"
                    f"Các mã trùng tên: {danh_sach_ma}"
                )
            challenge_match = trung_ten[0]

        nguoi_dung = await db.users.find_one({"_id": ctx.author.id}) or {}
        if challenge_match["_id"] in nguoi_dung.get("beaten_challenges", []):
            return await ctx.send(f"⚠️ Bạn đã được ghi nhận **vượt qua** challenge **{challenge_match['name']}** (`#{challenge_match['_id']}`) rồi, không thể nộp lại!")

        admin_channel = bot.get_channel(ADMIN_CHANNEL_ID)
        if admin_channel is None:
            return await ctx.send("❌ Không tìm thấy kênh admin để gửi duyệt (kiểm tra lại ADMIN_CHANNEL_ID và quyền bot).")

        embed = discord.Embed(title="🏁 YÊU CẦU DUYỆT BEAT CHALLENGE", color=discord.Color.orange())
        embed.set_thumbnail(url=ctx.author.display_avatar.url)
        embed.add_field(name="👤 Người nộp", value=f"{ctx.author.mention}\n({ctx.author.display_name} | `{ctx.author.id}`)", inline=False)
        embed.add_field(name="🎯 Challenge", value=f"**{challenge_match['name']}** (`#{challenge_match['_id']}`)", inline=True)
        embed.add_field(name="✨ Phần thưởng", value=f"**+{CP_PER_BEAT} CP**", inline=True)
        ...
        if not la_video_link and evidence_url:
            try:
                ...
                print(f"[LỖI TẢI TRƯỚC BẰNG CHỨNG !duyet BEAT CHALLENGE] {e}")
        ...
        embed.set_footer(text="Bấm Duyệt để cộng CP, hoặc Từ chối để nhập lý do")

        sent_msg = await admin_channel.send(embed=embed, view=ChallengeBeatReviewView(), file=evidence_file if evidence_file else discord.utils.MISSING)
        await db.pending_actions.insert_one({
            "_id": sent_msg.id,
            "kind": "beat_challenge",
            "user_id": ctx.author.id,
            "challenge_id": challenge_match["_id"],
            "challenge_name": challenge_match["name"],
        })
        _beat_challenge_cooldown[ctx.author.id] = now_ts
        return await ctx.send(f"✅ Đã gửi yêu cầu duyệt **vượt qua** challenge **{challenge_match['name']}** (`#{challenge_match['_id']}`) cho Admin xem xét!")
...
class ChallengeRankModal(Modal, title="Duyệt & Xếp hạng Challenge"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # ĐÃ THÊM MỚI: TỰ ĐỘNG cộng CP cho người Verify (750 = 500 beat + 250 bonus, vì Verify
        # nghĩa là họ cũng đã tự vượt qua challenge để xác nhận) và CCP cho người tạo level
        # (250), CHỈ khi được tag bằng @mention thật lúc !nopchallenge (verifier_id/creator_id
        # khác None). Chỉ chạy DUY NHẤT tại đây (lần duyệt ĐẦU TIÊN) - !bxhchallenge xếp lại
        # hạng sau này KHÔNG đi qua đường này nên không bị cộng điểm trùng lặp.
        award_log = []
        verifier_id = challenge.get("verifier_id")
        if verifier_id:
            v_user = await db.users.find_one({"_id": verifier_id}) or {}
            if self.challenge_id not in v_user.get("verified_challenges", []):
                await db.users.update_one(
                    {"_id": verifier_id},
                    {"$addToSet": {"verified_challenges": self.challenge_id, "beaten_challenges": self.challenge_id},
                     "$inc": {"cp": CP_PER_BEAT + CP_VERIFY_BONUS}},
                    upsert=True
                )
                await check_and_transfer_top1_cp(interaction.guild)
                new_titles_v = await check_and_grant_challenge_titles(verifier_id)
                v_msg = f"✅ Bạn được ghi nhận là **người Verify** challenge **{challenge['name']}** (`#{self.challenge_id}`)! +{CP_PER_BEAT + CP_VERIFY_BONUS} CP"
                if new_titles_v:
                    v_msg += "\n🎖️ Bạn vừa mở khoá danh hiệu mới: " + ", ".join(t.title() for t in new_titles_v)

                # ĐÃ THÊM MỚI: chuỗi Verify liên tiếp với ĐỘ KHÓ TĂNG DẦN (giống cơ chế Vua
                # Try Hard nhưng cho Challenge). Rank càng NHỎ = challenge càng KHÓ trên BXH
                # Challenge, nên streak chỉ tăng nếu challenge lần này có rank NHỎ HƠN lần
                # Verify liền trước; ngược lại (dễ hơn/bằng) thì streak reset về 1 (bản thân
                # lần Verify này vẫn tính là 1, không mất trắng).
                v_user_lai = await db.users.find_one({"_id": verifier_id}) or {}
                hang_truoc = v_user_lai.get("verify_last_rank")
                streak_hien_tai = v_user_lai.get("verify_streak", 0)
                if hang_truoc is not None and rank < hang_truoc:
                    streak_hien_tai += 1
                else:
                    streak_hien_tai = 1
                await db.users.update_one({"_id": verifier_id}, {"$set": {"verify_streak": streak_hien_tai, "verify_last_rank": rank}})
                if streak_hien_tai >= 5:
                    await grant_vua_verify_challenge(interaction.guild, verifier_id)
                    v_msg += f"\n👑 Chuỗi Verify tăng dần độ khó đạt **{streak_hien_tai} liên tiếp** - bạn nhận danh hiệu **Vua Verify Challenge**!"

                try:
                    vu = await bot.fetch_user(verifier_id)
                    if vu: await vu.send(v_msg)
                except discord.Forbidden:
                    pass
                award_log.append(f"Verify <@{verifier_id}>: +{CP_PER_BEAT + CP_VERIFY_BONUS} CP (streak {streak_hien_tai})")

        creator_id = challenge.get("creator_id")
        if creator_id:
            c_user = await db.users.find_one({"_id": creator_id}) or {}
            if self.challenge_id not in c_user.get("created_challenges", []):
                await db.users.update_one(
                    {"_id": creator_id},
                    {"$addToSet": {"created_challenges": self.challenge_id}, "$inc": {"ccp": CCP_PER_CREATE}},
                    upsert=True
                )
                new_titles_c = await check_and_grant_challenge_titles(creator_id)
                c_msg = f"🛠️ Bạn được ghi nhận là **người tạo** challenge **{challenge['name']}** (`#{self.challenge_id}`)! +{CCP_PER_CREATE} CCP"
                if new_titles_c:
                    c_msg += "\n🎖️ Bạn vừa mở khoá danh hiệu mới: " + ", ".join(t.title() for t in new_titles_c)
                try:
                    cu = await bot.fetch_user(creator_id)
                    if cu: await cu.send(c_msg)
                except discord.Forbidden:
                    pass
                award_log.append(f"Tạo <@{creator_id}>: +{CCP_PER_CREATE} CCP")

        if award_log:
            await log_admin_action(interaction.user.mention, f"Tự động cộng CP/CCP cho Challenge #{self.challenge_id}", None, "\n".join(award_log))
...
# ================= THÊM MỚI: DUYỆT "BEAT CHALLENGE" (KHÁC VỚI DUYỆT NỘP CHALLENGE MỚI) ================= #
# Khác với !nopchallenge (đề xuất 1 challenge MỚI lên BXH), đây là luồng dành cho BẤT KỲ
# ai muốn được ghi nhận đã VƯỢT QUA (beat) 1 challenge ĐÃ CÓ SẴN trên BXH Challenge, qua
# lệnh `!duyet [mã hoặc tên challenge]` kèm bằng chứng (xem thêm ở lệnh duyet()). Mỗi lượt
# duyệt thành công luôn cố định CP_PER_BEAT (500) CP, không phân biệt độ khó challenge.
class BeatChallengeRejectModal(Modal, title="Từ chối Beat Challenge"):
    reason = TextInput(label="Lý do từ chối", style=discord.TextStyle.paragraph, required=True)

    def __init__(self, user_id: int, challenge_id: int, challenge_name: str, message_to_edit):
        super().__init__()
        self.user_id = user_id
        self.challenge_id = challenge_id
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await log_history(self.user_id, "cp", self.challenge_name, "rejected", interaction.user.id, self.reason.value)
        await log_admin_action(interaction.user.mention, f"Từ chối Beat Challenge #{self.challenge_id}: **{self.challenge_name}**", f"<@{self.user_id}>", self.reason.value)

        user = await bot.fetch_user(self.user_id)
        if user:
            try: await user.send(f"❌ Yêu cầu ghi nhận **vượt qua** challenge **{self.challenge_name}** (`#{self.challenge_id}`) của bạn bị từ chối.\n**Lý do:** {self.reason.value}")
            except discord.Forbidden: pass

        await self.message_to_edit.edit(content=f"❌ **Đã từ chối Beat Challenge** `#{self.challenge_id}` — **{self.challenge_name}** của <@{self.user_id}>\nLý do: {self.reason.value}\nBởi: {interaction.user.mention}", view=None, embeds=[])
        await db.pending_actions.delete_one({"_id": self.message_to_edit.id})
        await interaction.response.send_message("Đã thông báo từ chối.", ephemeral=True)

class ChallengeBeatReviewView(View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Duyệt", style=discord.ButtonStyle.green, custom_id="btn_beat_approve")
    async def approve_btn(self, interaction: discord.Interaction, button: Button):
        # ĐÃ FIX (giống ReviewView.approve_btn): dùng find_one_and_delete để "claim" atomic,
        # tránh cộng CP gấp đôi nếu admin bấm Duyệt 2 lần liên tiếp/lag.
        data = await db.pending_actions.find_one_and_delete({"_id": interaction.message.id})
        if not data:
            return await interaction.response.send_message("⚠️ Yêu cầu này đã được xử lý rồi (có thể do bấm trùng hoặc admin khác đã duyệt).", ephemeral=True)

        await interaction.response.defer(ephemeral=True, thinking=True)
        user_id, challenge_id, challenge_name = data["user_id"], data["challenge_id"], data["challenge_name"]

        user_doc = await db.users.find_one({"_id": user_id}) or {}
        if challenge_id in user_doc.get("beaten_challenges", []):
            await interaction.message.edit(content=f"⚠️ <@{user_id}> đã được ghi nhận vượt qua challenge `#{challenge_id}` từ trước - không cộng CP lần nữa.", view=None, embeds=[])
            return await interaction.followup.send("⚠️ Người này đã được ghi nhận vượt qua challenge này rồi, không cộng CP lần nữa.", ephemeral=True)

        await db.users.update_one({"_id": user_id}, {"$addToSet": {"beaten_challenges": challenge_id}, "$inc": {"cp": CP_PER_BEAT}}, upsert=True)
        await check_and_transfer_top1_cp(interaction.guild)
        new_titles = await check_and_grant_challenge_titles(user_id)

        await log_history(user_id, "cp", challenge_name, "approved", interaction.user.id, f"+{CP_PER_BEAT} CP (beat challenge #{challenge_id})")
        await log_admin_action(interaction.user.mention, f"Duyệt Beat Challenge #{challenge_id}: **{challenge_name}**", f"<@{user_id}>", f"+{CP_PER_BEAT} CP")

        msg = f"🎉 Bạn đã được ghi nhận **vượt qua** challenge **{challenge_name}** (`#{challenge_id}`)! +{CP_PER_BEAT} CP"
        if new_titles:
            msg += "\n🎖️ Bạn vừa mở khoá danh hiệu mới: " + ", ".join(t.title() for t in new_titles)
        user = await bot.fetch_user(user_id)
        if user:
            try: await user.send(msg)
            except discord.Forbidden: pass

        await interaction.message.edit(content=f"✅ **Đã duyệt Beat Challenge** `#{challenge_id}` — **{challenge_name}** cho <@{user_id}> (+{CP_PER_BEAT} CP)\nBởi: {interaction.user.mention}", view=None, embeds=[])
        await interaction.followup.send("✅ Đã duyệt thành công!", ephemeral=True)

    @discord.ui.button(label="Từ chối", style=discord.ButtonStyle.red, custom_id="btn_beat_reject")
    async def reject_btn(self, interaction: discord.Interaction, button: Button):
        data = await db.pending_actions.find_one({"_id": interaction.message.id})
        if not data:
            return await interaction.response.send_message("⚠️ Không tìm thấy dữ liệu yêu cầu này (có thể đã được xử lý bởi admin khác).", ephemeral=True)
        await interaction.response.send_modal(BeatChallengeRejectModal(data["user_id"], data["challenge_id"], data["challenge_name"], interaction.message))
# ==================================================================================================== #
...
async def nopchallenge(ctx, level_id: str = None, *, noi_dung: str = None):
    ...
    # ĐÃ THÊM MỚI: nếu người dùng gõ @ rồi chọn tên thật ở trường Verify/người tạo (Discord
    # tự chuyển thành mention <@id>), bot dò ra đúng user ID để sau này TỰ ĐỘNG cộng
    # CP (Verify)/CCP (người tạo) khi challenge được Admin duyệt lần đầu - xem
    # ChallengeRankModal.on_submit. Nếu chỉ gõ tên chữ thường (không phải mention thật)
    # thì verifier_id/creator_id sẽ là None -> không tự động cộng điểm được.
    verifier_id = extract_user_id(ten_verify)
    creator_id = extract_user_id(ten_tao)
...
async def bxhchallenge(ctx, challenge_id: int = None, vi_tri: int = None):
    ...
    # ĐÃ SỬA (v28): layout cũ gộp hết vào 1-2 dòng dài (tên + ID level + tạo + verify +
    # mention nộp bởi) khiến chữ bị dồn/ngắt dòng lung tung trên mobile, đặc biệt khi tên
    # người tạo/Verify có chứa mention thật (dạng <@id> do gõ @ rồi bấm chọn tên trên bàn
    # phím Discord) -> pill mention chiếm nhiều bề ngang làm vỡ layout. Giờ mỗi challenge
    # là 1 khối blockquote (">") riêng, mỗi dòng ngắn 1 thông tin, và dùng display_name
    # (tên hiển thị) thay vì mention pill cho "nộp bởi" để đỡ rối + đỡ chiếm chỗ.
    def khoi_hien_thi(c, hien_hang: bool):
        nguoi_nop = ctx.guild.get_member(c["submitter_id"]) if ctx.guild else None
        ten_nop = nguoi_nop.display_name if nguoi_nop else f"ID `{c['submitter_id']}`"
        tieu_de = f"**#{c['rank']} · {c['name']}**" if hien_hang else f"**{c['name']}**"
        return (
            f"{tieu_de}\n"
            f"> 🎮 Level `{c.get('level_id','?')}` · Mã `#{c['_id']}`\n"
            f"> {format_creator_verifier(c)}\n"
            f"> 👤 Nộp bởi: {ten_nop}"
        )

    # ĐÃ THÊM MỚI (v28): tự chia thành nhiều field nếu danh sách dài quá giới hạn 1024
    # ký tự/field của Discord, thay vì cắt cụt ([:1024] cũ) làm mất dữ liệu cuối bảng.
    def chia_field(embed_, ten_field, khoi_list):
        if not khoi_list: return
        buf, idx = "", 1
        for khoi in khoi_list:
            phan_moi = (khoi if not buf else buf + "\n\n" + khoi)
            if len(phan_moi) > 1000 and buf:
                embed_.add_field(name=ten_field if idx == 1 else f"{ten_field} (tiếp)", value=buf, inline=False)
                buf, idx = khoi, idx + 1
            else:
                buf = phan_moi
        if buf:
    ...
    chia_field(embed, "🏆 Đã xếp hạng", [khoi_hien_thi(c, True) for c in ranked])
    chia_field(embed, "⏳ Đã duyệt, chờ Admin xếp hạng", [khoi_hien_thi(c, False) for c in unranked])
...
# ĐÃ THÊM MỚI (v27): cho Admin sửa lại thông tin 1 challenge đã nộp (lỡ gõ sai tên, sai
# tên người Verify/người tạo...) mà không cần bắt member xoá nộp lại từ đầu (mất rank cũ).
# Cú pháp giống !nopchallenge, dùng dấu "|" để tách 3 trường:
#   !editbxhchallenge [id] [tên] | [tên người Verify] | [tên creator]
# Cho phép gõ dấu "-" ở 1 trường bất kỳ để GIỮ NGUYÊN giá trị cũ, khỏi phải gõ lại cả 3
# khi chỉ muốn sửa đúng 1 trường (VD chỉ sửa tên Verify: "!editbxhchallenge 5 - | Tên đúng | -").
@bot.command(aliases=["editchallenge", "suachallenge"])
@commands.has_permissions(administrator=True)
@require_db()
async def editbxhchallenge(ctx, challenge_id: int = None, *, noi_dung: str = None):
    USAGE = ("❌ VD: `!editbxhchallenge 5 Tên Challenge mới | Tên người Verify | Tên người tạo`\n"
              "Gõ dấu `-` ở 1 trường để GIỮ NGUYÊN giá trị cũ, VD chỉ sửa tên Verify:\n"
              "`!editbxhchallenge 5 - | Tên Verify đúng | -`")
    if not challenge_id or not noi_dung:
        return await ctx.send(USAGE)

    phan = [p.strip() for p in noi_dung.split("|")]
    if len(phan) < 3:
        return await ctx.send(USAGE + "\n⚠️ Nhớ tách đủ 3 trường bằng dấu `|` (dùng `-` cho trường muốn giữ nguyên).")

    challenge = await db.challenges.find_one({"_id": challenge_id})
    if not challenge: return await ctx.send(f"❌ Không tìm thấy challenge `#{challenge_id}`.")
    if challenge["status"] == "removed":
        return await ctx.send(f"❌ Challenge `#{challenge_id}` đã bị xoá khỏi hệ thống, không thể sửa.")

    ten_moi, verify_moi, creator_moi = phan[0], phan[1], phan[2]
    cap_nhat = {}
    thay_doi = []

    if ten_moi and ten_moi != "-":
        ten_moi = strip_youtube_link(ten_moi)
        if not ten_moi:
            return await ctx.send("❌ Tên challenge không được để trống (sau khi bỏ link Youtube).")
        if ten_moi != challenge.get("name"):
            thay_doi.append(f"Tên: `{challenge.get('name')}` → `{ten_moi}`")
        cap_nhat["name"] = ten_moi

    if verify_moi and verify_moi != "-":
        if verify_moi != challenge.get("verifier_name"):
            thay_doi.append(f"Verify: `{challenge.get('verifier_name', '?')}` → `{verify_moi}`")
        cap_nhat["verifier_name"] = verify_moi
        # ĐÃ THÊM MỚI: cập nhật lại verifier_id theo mention mới (nếu có). LƯU Ý: nếu
        # challenge NÀY đã từng được duyệt và CP đã cộng cho người Verify CŨ, sửa lại
        # ở đây KHÔNG tự động thu hồi/chuyển CP - chỉ đổi thông tin hiển thị/để lần
        # duyệt TIẾP THEO (nếu có) tính đúng người. Admin dùng !addcp/!setcp để chỉnh
        # tay CP nếu cần thu hồi/chuyển lại.
        cap_nhat["verifier_id"] = extract_user_id(verify_moi)

    if creator_moi and creator_moi != "-":
        if creator_moi != challenge.get("creator_name"):
            thay_doi.append(f"Người tạo: `{challenge.get('creator_name', '?')}` → `{creator_moi}`")
        cap_nhat["creator_name"] = creator_moi
        # ĐÃ THÊM MỚI: xem chú thích tương tự ở verifier_id phía trên (không tự thu hồi CCP cũ).
        cap_nhat["creator_id"] = extract_user_id(creator_moi)

    if not cap_nhat:
        return await ctx.send("⚠️ Không có trường nào để sửa (cả 3 đều là `-`).")

    await db.challenges.update_one({"_id": challenge_id}, {"$set": cap_nhat})
    await log_history(challenge["submitter_id"], "challenge", cap_nhat.get("name", challenge["name"]), "edited", ctx.author.id, "; ".join(thay_doi))
    await log_admin_action(ctx.author.mention, f"Sửa Challenge #{challenge_id}", f"<@{challenge['submitter_id']}>", "\n".join(thay_doi) if thay_doi else "Không có gì thay đổi")

    await ctx.send(f"✅ Đã cập nhật challenge `#{challenge_id}`:\n" + "\n".join(thay_doi) + "\n\n⚠️ Lưu ý: tin nhắn duyệt CŨ ở kênh admin (nếu còn) sẽ không tự cập nhật lại, chỉ dữ liệu trong hệ thống/BXH là được sửa.")
# ==================================================================== #

# ================= THÊM MỚI: BẢNG XẾP HẠNG CP & CCP ================= #
@bot.command(aliases=["cpboard", "bxhchallengepoint"])
@require_db()
async def bxhcp(ctx):
    users = await db.users.find({"cp": {"$gt": 0}}).sort("cp", -1).limit(20).to_list(20)
    if not users: return await ctx.send("Chưa có ai sở hữu CP (Challenge Point) trên server!")
    guild = ctx.guild
    dong = []
    for i, u in enumerate(users, start=1):
        m = guild.get_member(u["_id"]) if guild else None
        if not m: m = bot.get_user(u["_id"])
        ten = m.display_name if m else f"ID `{u['_id']}`"
        dong.append(f"**#{i}** — {ten} — `{u.get('cp', 0)} CP`")
    embed = discord.Embed(title="🏁 BẢNG XẾP HẠNG CP (CHALLENGE POINT)", description="\n".join(dong), color=discord.Color.orange())
    embed.set_footer(text="CP: +500 khi Beat 1 challenge trên BXH (!duyet [mã/tên]), +750 nếu được tag làm người Verify (500 beat + 250 bonus).")
    await ctx.send(embed=embed)

@bot.command(aliases=["ccpboard"])
@require_db()
async def bxhccp(ctx):
    users = await db.users.find({"ccp": {"$gt": 0}}).sort("ccp", -1).limit(20).to_list(20)
    if not users: return await ctx.send("Chưa có ai sở hữu CCP (Creator Challenge Point) trên server!")
    guild = ctx.guild
    dong = []
    for i, u in enumerate(users, start=1):
        m = guild.get_member(u["_id"]) if guild else None
        if not m: m = bot.get_user(u["_id"])
        ten = m.display_name if m else f"ID `{u['_id']}`"
        dong.append(f"**#{i}** — {ten} — `{u.get('ccp', 0)} CCP`")
    embed = discord.Embed(title="🛠️ BẢNG XẾP HẠNG CCP (CREATOR CHALLENGE POINT)", description="\n".join(dong), color=discord.Color.teal())
    embed.set_footer(text="CCP: +250 khi được tag làm người tạo level ở 1 challenge được duyệt (qua !nopchallenge).")
    await ctx.send(embed=embed)

# ĐÃ THÊM MỚI: !bxhtong - gộp MP + CP + CCP thành 1 "điểm uy tín" chung, xếp hạng toàn
# năng của server (không thay thế !bxh/!bxhcp/!bxhccp riêng lẻ, chỉ để xem tổng quan).
@bot.command(aliases=["bxhuytin", "totalboard", "uytin"])
@require_db()
async def bxhtong(ctx):
    users = await db.users.find({"$or": [{"mp": {"$gt": 0}}, {"cp": {"$gt": 0}}, {"ccp": {"$gt": 0}}]}).to_list(None)
    if not users: return await ctx.send("Chưa có ai có điểm nào trên server!")

    xep_hang = []
    for u in users:
        mp, cp, ccp = u.get("mp", 0), u.get("cp", 0), u.get("ccp", 0)
        xep_hang.append((mp + cp + ccp, mp, cp, ccp, u["_id"]))
    xep_hang.sort(key=lambda x: x[0], reverse=True)
    xep_hang = xep_hang[:20]

    guild = ctx.guild
    dong = []
    for i, (tong, mp, cp, ccp, uid) in enumerate(xep_hang, start=1):
        m = guild.get_member(uid) if guild else None
        if not m: m = bot.get_user(uid)
        ten = m.display_name if m else f"ID `{uid}`"
        dong.append(f"**#{i}** — {ten} — `{tong} điểm` (MP {mp} | CP {cp} | CCP {ccp})")

    embed = discord.Embed(title="👑 BẢNG XẾP HẠNG UY TÍN TỔNG (MP + CP + CCP)", description="\n".join(dong), color=discord.Color.gold())
    embed.set_footer(text="Điểm uy tín = MP + CP + CCP cộng dồn, chỉ để xem tổng quan - KHÔNG ảnh hưởng tới hạng/danh hiệu riêng của từng loại điểm.")
    await ctx.send(embed=embed)
# ==================================================================== #

# ĐÃ THÊM MỚI: !challengeinfo [mã] - xem chi tiết 1 challenge: ai tạo, ai Verify, hạng,
# trạng thái, và có bao nhiêu người đã được ghi nhận Beat challenge đó.
@bot.command(aliases=["challinfo", "thongtinchallenge"])
@require_db()
async def challengeinfo(ctx, challenge_id: int = None):
    if challenge_id is None:
        return await ctx.send("❌ VD: `!challengeinfo 5` (xem chi tiết challenge mã `#5`)")

    challenge = await db.challenges.find_one({"_id": challenge_id})
    if not challenge:
        return await ctx.send(f"❌ Không tìm thấy challenge với mã `#{challenge_id}`.")

    so_nguoi_beat = await db.users.count_documents({"beaten_challenges": challenge_id})

    trang_thai_hien_thi = {
        "pending": "⏳ Đang chờ duyệt", "approved": "✅ Đã duyệt",
        "rejected": "❌ Bị từ chối", "removed": "🚫 Đã bị gỡ",
    }.get(challenge.get("status"), challenge.get("status", "?"))

    embed = discord.Embed(title=f"🏁 Thông tin Challenge #{challenge_id}", color=discord.Color.orange())
    embed.add_field(name="🏷️ Tên", value=challenge.get("name", "?"), inline=True)
    embed.add_field(name="🎮 ID Level", value=f"`{challenge.get('level_id', '?')}`", inline=True)
    embed.add_field(name="📌 Trạng thái", value=trang_thai_hien_thi, inline=True)
    if challenge.get("rank"):
        embed.add_field(name="🏆 Hạng trên BXH", value=f"#{challenge['rank']}", inline=True)
    embed.add_field(name="👤 Người nộp", value=f"<@{challenge['submitter_id']}>", inline=True)
    embed.add_field(name="🛠️ Người tạo", value=challenge.get("creator_name", "?"), inline=True)
    embed.add_field(name="✅ Người Verify", value=challenge.get("verifier_name", "?"), inline=True)
    embed.add_field(name="🎯 Số người đã Beat (qua !duyet)", value=str(so_nguoi_beat), inline=True)
    if challenge.get("evidence_url"):
        embed.add_field(name="🔗 Bằng chứng gốc (lúc nộp)", value=f"[Xem tại đây]({challenge['evidence_url']})", inline=False)
    await ctx.send(embed=embed)
# ==================================================================== #
# ==================================================================== #

# ================= THÊM MỚI: LỆNH ADMIN QUẢN LÝ CP/CCP ================= #
@bot.command()
@commands.has_permissions(administrator=True)
@require_db()
async def setcp(ctx, member: discord.Member, amount: int):
    await db.users.update_one({"_id": member.id}, {"$set": {"cp": amount}}, upsert=True)
    await check_and_transfer_top1_cp(ctx.guild)
    await log_admin_action(ctx.author.mention, "Đặt lại CP", member.mention, f"Đặt thành {amount} CP")
    await ctx.send(f"✅ Đã đặt CP của {member.mention} thành {amount}.")

@bot.command()
@commands.has_permissions(administrator=True)
@require_db()
async def addcp(ctx, member: discord.Member, amount: int):
    await add_user_cp(member.id, amount, ctx.guild)
    await log_admin_action(ctx.author.mention, "Cộng/trừ CP", member.mention, f"{'Cộng' if amount > 0 else 'Trừ'} {abs(amount)} CP")
    await ctx.send(f"✅ Đã {'cộng' if amount > 0 else 'trừ'} {abs(amount)} CP cho {member.mention}.")

@bot.command()
@commands.has_permissions(administrator=True)
@require_db()
async def setccp(ctx, member: discord.Member, amount: int):
    await db.users.update_one({"_id": member.id}, {"$set": {"ccp": amount}}, upsert=True)
    await log_admin_action(ctx.author.mention, "Đặt lại CCP", member.mention, f"Đặt thành {amount} CCP")
    await ctx.send(f"✅ Đã đặt CCP của {member.mention} thành {amount}.")

@bot.command()
@commands.has_permissions(administrator=True)
@require_db()
async def addccp(ctx, member: discord.Member, amount: int):
    await add_user_ccp(member.id, amount)
    await log_admin_action(ctx.author.mention, "Cộng/trừ CCP", member.mention, f"{'Cộng' if amount > 0 else 'Trừ'} {abs(amount)} CCP")
    await ctx.send(f"✅ Đã {'cộng' if amount > 0 else 'trừ'} {abs(amount)} CCP cho {member.mention}.")

# ĐÃ THÊM MỚI: cho Admin thủ công thêm/gỡ 1 lượt "beat challenge" (VD: member báo bằng
# chứng qua kênh khác/off-Discord, hoặc cần sửa sai sót), thay vì bắt họ !duyet lại.
@bot.command(aliases=["addbeat"])
@commands.has_permissions(administrator=True)
@require_db()
async def themluotbeat(ctx, member: discord.Member, challenge_id: int):
    challenge = await db.challenges.find_one({"_id": challenge_id, "status": "approved"})
    if not challenge: return await ctx.send(f"❌ Không tìm thấy challenge đã duyệt với mã `#{challenge_id}`.")
    user = await db.users.find_one({"_id": member.id}) or {}
    if challenge_id in user.get("beaten_challenges", []):
        return await ctx.send(f"⚠️ {member.mention} đã được ghi nhận vượt qua challenge này rồi.")
    await db.users.update_one({"_id": member.id}, {"$addToSet": {"beaten_challenges": challenge_id}, "$inc": {"cp": CP_PER_BEAT}}, upsert=True)
    await check_and_transfer_top1_cp(ctx.guild)
    new_titles = await check_and_grant_challenge_titles(member.id)
    chi_tiet = f"+{CP_PER_BEAT} CP" + (f"\n🎖️ Danh hiệu mới: {', '.join(t.title() for t in new_titles)}" if new_titles else "")
    await log_admin_action(ctx.author.mention, f"Thêm lượt Beat Challenge #{challenge_id} (thủ công)", member.mention, chi_tiet)
    await ctx.send(f"✅ Đã ghi nhận {member.mention} vượt qua challenge **{challenge['name']}** (`#{challenge_id}`), +{CP_PER_BEAT} CP.")

@bot.command(aliases=["removebeat"])
@commands.has_permissions(administrator=True)
@require_db()
async def xoaluotbeat(ctx, member: discord.Member, challenge_id: int):
    user = await db.users.find_one({"_id": member.id}) or {}
    if challenge_id not in user.get("beaten_challenges", []):
        return await ctx.send(f"⚠️ {member.mention} chưa được ghi nhận vượt qua challenge `#{challenge_id}`.")
    await db.users.update_one({"_id": member.id}, {"$pull": {"beaten_challenges": challenge_id}, "$inc": {"cp": -CP_PER_BEAT}})
    await log_admin_action(ctx.author.mention, f"Xoá lượt Beat Challenge #{challenge_id} (thủ công)", member.mention, f"-{CP_PER_BEAT} CP")
    await ctx.send(f"✅ Đã gỡ ghi nhận vượt qua challenge `#{challenge_id}` của {member.mention}, -{CP_PER_BEAT} CP.")

# ĐÃ THÊM MỚI: vá dữ liệu khi CP/CCP đã được cộng (VD do admin test bằng !setcp/!addcp,
# hoặc dữ liệu cũ trước khi có cơ chế theo dõi) nhưng mảng verified_challenges/
# created_challenges (dùng để hiển thị "Đã beat/Verify/Tạo bao nhiêu challenge" ở !profile
# và để mở khoá danh hiệu) chưa được ghi nhận. 2 lệnh này CHỈ thêm vào mảng, KHÔNG cộng
# thêm CP/CCP (vì điểm coi như đã được cộng từ trước bằng cách khác).
@bot.command()
@commands.has_permissions(administrator=True)
@require_db()
async def fixverify(ctx, member: discord.Member, challenge_id: int):
    await db.users.update_one({"_id": member.id}, {"$addToSet": {"verified_challenges": challenge_id, "beaten_challenges": challenge_id}}, upsert=True)
    new_titles = await check_and_grant_challenge_titles(member.id)
    chi_tiet = ("Danh hiệu mới: " + ", ".join(t.title() for t in new_titles)) if new_titles else "Không có danh hiệu mới"
    await log_admin_action(ctx.author.mention, f"Vá dữ liệu Verify Challenge #{challenge_id} (KHÔNG cộng thêm CP)", member.mention, chi_tiet)
    await ctx.send(f"✅ Đã vá dữ liệu: {member.mention} được ghi nhận Verify + Beat challenge `#{challenge_id}` (không cộng thêm CP).")

@bot.command()
@commands.has_permissions(administrator=True)
@require_db()
async def fixcreate(ctx, member: discord.Member, challenge_id: int):
    await db.users.update_one({"_id": member.id}, {"$addToSet": {"created_challenges": challenge_id}}, upsert=True)
    new_titles = await check_and_grant_challenge_titles(member.id)
    chi_tiet = ("Danh hiệu mới: " + ", ".join(t.title() for t in new_titles)) if new_titles else "Không có danh hiệu mới"
    await log_admin_action(ctx.author.mention, f"Vá dữ liệu Tạo Challenge #{challenge_id} (KHÔNG cộng thêm CCP)", member.mention, chi_tiet)
    await ctx.send(f"✅ Đã vá dữ liệu: {member.mention} được ghi nhận là người tạo challenge `#{challenge_id}` (không cộng thêm CCP).")
# ==================================================================== #

# ================= THÊM MỚI: LỆNH ADMIN CẤP/GỠ DANH HIỆU THỦ CÔNG ================= #
# ĐÃ THÊM MỚI: gom TOÀN BỘ danh hiệu hợp lệ trong hệ thống vào 1 tập hợp DUY NHẤT để
# !adddanhhieu/!xoadanhhieu tra cứu - tránh xung đột/lệch dữ liệu với TITLES_DATA,
# CHALLENGE_TITLE_RULES và các danh hiệu độc quyền (vua cày điểm/vô song cp/song kiếm
# challenge/vua verify challenge) vốn nằm RIÊNG ngoài TITLES_DATA (không cho !duyet tay).
def _tat_ca_danh_hieu_hop_le():
    return set(TITLES_DATA) | {t for _, _, t in CHALLENGE_TITLE_RULES} | {
        "vua cày điểm", "vô song cp", "song kiếm challenge", "vua verify challenge",
    }

# Các danh hiệu ĐỘC QUYỀN (chỉ 1 người giữ tại 1 thời điểm) PHẢI đi qua đúng hàm grant_*
# tương ứng (tự động thu hồi của người cũ) - không được addToSet thô, nếu không sẽ có
# 2+ người cùng giữ 1 ngôi vị -> sai lệch toàn bộ hệ thống hiển thị/BXH.
DANH_HIEU_DOC_QUYEN_QUA_GRANT = {"vua hardest": grant_vua_hardest, "vua try hard": grant_vua_try_hard, "vua verify challenge": grant_vua_verify_challenge}
# 2 danh hiệu này KHÔNG thể gán tay vì được tính toán lại TỰ ĐỘNG mỗi khi MP/CP thay đổi
# (xem check_and_transfer_top1_mp/cp) - gán tay xong sẽ bị hệ thống tự "cướp lại" ngay khi
# có biến động điểm tiếp theo, gây hiểu lầm cho cả admin lẫn người dùng.
DANH_HIEU_TU_DONG_HOAN_TOAN = {"vua cày điểm": "!setmp/!addmp", "vô song cp": "!setcp/!addcp"}

@bot.command(aliases=["capdanhhieu", "grantdanhhieu", "adddanhieu"])
@commands.has_permissions(administrator=True)
@require_db()
async def adddanhhieu(ctx, member: discord.Member, *, noi_dung: str = None):
    if not noi_dung:
        return await ctx.send("❌ VD: `!adddanhhieu @user vua hardest | Tên Level Hardest` hoặc `!adddanhhieu @user hardcore player` (không có tên hardest thì bỏ qua phần `| ...`)")

    if "|" in noi_dung:
        ten_danh_hieu, ten_hardest = noi_dung.split("|", 1)
        ten_danh_hieu, ten_hardest = ten_danh_hieu.strip().lower(), ten_hardest.strip()
    else:
        ten_danh_hieu, ten_hardest = noi_dung.strip().lower(), None

    tat_ca = _tat_ca_danh_hieu_hop_le()
    if ten_danh_hieu not in tat_ca:
        goi_y = difflib.get_close_matches(ten_danh_hieu, tat_ca, n=3)
        goi_y_text = f"\n💡 Có phải bạn muốn nhập: {', '.join(f'`{g}`' for g in goi_y)}?" if goi_y else ""
        return await ctx.send(f"❌ Không tìm thấy danh hiệu **{ten_danh_hieu}** trong hệ thống.{goi_y_text}\nDùng `!danhsachdanhhieu` để xem đầy đủ danh sách.")

    if ten_danh_hieu in DANH_HIEU_TU_DONG_HOAN_TOAN:
        lenh_thay_the = DANH_HIEU_TU_DONG_HOAN_TOAN[ten_danh_hieu]
        return await ctx.send(f"❌ Danh hiệu **{ten_danh_hieu.title()}** được tính TỰ ĐỘNG theo Top 1 điểm, không thể gán tay (gán xong sẽ bị hệ thống tự thu hồi lại ngay khi có ai đổi điểm). Dùng `{lenh_thay_the}` để đẩy điểm người này lên Top 1 nếu muốn cấp.")

    if ten_danh_hieu in DANH_HIEU_DOC_QUYEN_QUA_GRANT:
        await DANH_HIEU_DOC_QUYEN_QUA_GRANT[ten_danh_hieu](ctx.guild, member.id)
    else:
        await db.users.update_one({"_id": member.id}, {"$addToSet": {"titles": ten_danh_hieu}, "$set": {"active_title": ten_danh_hieu}}, upsert=True)

    chi_tiet = f"Cấp thủ công danh hiệu **{ten_danh_hieu.title()}**" + (f" (Hardest: **{ten_hardest}**)" if ten_hardest else "")
    await log_admin_action(ctx.author.mention, "Cấp danh hiệu (thủ công)", member.mention, chi_tiet)
    await log_history(member.id, "role", ten_danh_hieu, "approved", ctx.author.id, chi_tiet)

    # ĐÃ THÊM MỚI: trước đây chỉ gửi DM chữ trơn + reply admin, KHÔNG có thông báo công khai
    # nào ở kênh welcome như lúc tự động nhận danh hiệu (VD 10 danh hiệu Challenge/CP/CCP) -
    # khiến admin cấp tay xong "cảm giác không có gì xảy ra". Giờ đăng embed công khai giống
    # hệt kiểu thông báo danh hiệu tự động, để cả server thấy và ăn mừng cùng.
    mo_ta_cong_khai = f"{member.mention} vừa được cấp danh hiệu **{ten_danh_hieu.title()}**!"
    if ten_hardest:
        mo_ta_cong_khai += f"\n🏔️ Hardest: **{ten_hardest}**"
    thong_bao_channel = bot.get_channel(WELCOME_CHANNEL_ID)
    if thong_bao_channel:
        embed_tb = discord.Embed(title="🎖️ DANH HIỆU MỚI!", description=mo_ta_cong_khai, color=discord.Color.gold())
        try:
            await thong_bao_channel.send(embed=embed_tb)
        except (discord.Forbidden, discord.HTTPException):
            pass

    embed_dm = discord.Embed(title="🎖️ Bạn vừa nhận danh hiệu mới!", description=f"Admin đã cấp cho bạn danh hiệu **{ten_danh_hieu.title()}**!" + (f"\n🏔️ Hardest: **{ten_hardest}**" if ten_hardest else ""), color=discord.Color.gold())
    try:
        await member.send(embed=embed_dm)
    except discord.Forbidden:
        pass

    embed_admin = discord.Embed(title="✅ Đã cấp danh hiệu", description=chi_tiet + f" cho {member.mention}.", color=discord.Color.green())
    await ctx.send(embed=embed_admin)

# ĐÃ THÊM MỚI: gỡ danh hiệu thủ công (đối xứng với !adddanhhieu) - dùng revoke_title() có
# sẵn để tự chuyển active_title sang danh hiệu khác nếu người đó đang trang bị đúng cái bị
# gỡ, tránh để active_title trỏ tới 1 danh hiệu đã không còn trong tủ đồ.
@bot.command(aliases=["xoadanhhieuadmin", "removedanhhieu", "xoadanhieu"])
@commands.has_permissions(administrator=True)
@require_db()
async def xoadanhhieu(ctx, member: discord.Member, *, ten_danh_hieu: str = None):
    if not ten_danh_hieu:
        return await ctx.send("❌ VD: `!xoadanhhieu @user hardcore player`")
    ten_danh_hieu = ten_danh_hieu.strip().lower()

    if ten_danh_hieu in DANH_HIEU_TU_DONG_HOAN_TOAN:
        return await ctx.send(f"⚠️ Danh hiệu **{ten_danh_hieu.title()}** do hệ thống tự quản lý theo Top 1 điểm - nếu người này vẫn đang Top 1, hệ thống sẽ tự cấp lại ngay khi có biến động điểm tiếp theo. Muốn gỡ hẳn thì phải hạ điểm của họ xuống dưới người thứ 2.")

    user_doc = await db.users.find_one({"_id": member.id}) or {}
    if ten_danh_hieu not in user_doc.get("titles", []):
        return await ctx.send(f"⚠️ {member.mention} hiện không sở hữu danh hiệu **{ten_danh_hieu.title()}**.")

    await revoke_title(member.id, ten_danh_hieu)
    await log_admin_action(ctx.author.mention, "Gỡ danh hiệu (thủ công)", member.mention, f"Gỡ **{ten_danh_hieu.title()}**")
    await log_history(member.id, "role", ten_danh_hieu, "rejected", ctx.author.id, "Bị Admin gỡ danh hiệu thủ công")
    try:
        await member.send(f"⚠️ Danh hiệu **{ten_danh_hieu.title()}** của bạn đã bị Admin gỡ.")
    except discord.Forbidden:
        pass
    await ctx.send(f"✅ Đã gỡ danh hiệu **{ten_danh_hieu.title()}** của {member.mention}.")
# ==================================================================== #
...
async def danhsachdanhhieu(ctx):
    ...
    embed2.add_field(name="🏁 Challenge/CP/CCP (10 danh hiệu MỚI - TỰ ĐỘNG NHẬN, không duyệt tay)", value=(
        "**Thử Thách Đầu Tiên** — Beat 1 challenge bất kỳ trên BXH Challenge (sở hữu 500 CP)\n"
        "**Thử Thách Mới** — Beat 10 challenge trên BXH (sở hữu 5.000 CP)\n"
        "**Huyền Thoại Challenge** — Beat 50 challenge trên BXH\n"
        "**Vua Challenge** — Beat 100 challenge trên BXH (sở hữu 50.000 CP)\n"
        "**Thần Challenge** — Được tag làm người Verify ở 5 challenge trên BXH\n"
        "**Bậc Thầy Verify** — Được tag làm người Verify ở 15 challenge trên BXH\n"
        "**Người Mở Đường** — Được tag làm người tạo level ở 5 challenge trên BXH\n"
        "**Kiến Trúc Sư Challenge** — Được tag làm người tạo level ở 20 challenge trên BXH\n"
        "**Song Kiếm Challenge** — Vừa Beat ≥5 vừa được Verify ≥5 challenge\n"
        "**Vô Song CP** — Top 1 CP toàn server *(độc quyền 1 người, mất khi bị vượt mặt - giống Vua Cày Điểm)*\n"
        "**Vua Verify Challenge** — Verify liên tiếp ≥5 challenge với ĐỘ KHÓ TĂNG DẦN (hạng sau phải khó hơn hạng trước) *(độc quyền 1 người, giống Vua Hardest)*\n"
        "📌 Beat 1 challenge có sẵn: `!duyet [mã/tên challenge]` kèm bằng chứng. Verify/tạo level: được tag @ ở !nopchallenge.\n"
        "📌 `!bxhcp`/`!bxhccp` xem BXH riêng · `!bxhtong` xem BXH uy tín tổng (MP+CP+CCP) · `!challengeinfo [mã]` xem chi tiết 1 challenge."
    ), inline=False)
    embed2.set_footer(text=f"Tổng cộng {len(TITLES_DATA) + 11} danh hiệu (gồm 11 danh hiệu Challenge/CP/CCP) | Dùng !listdanhhieu để xem danh hiệu bạn đang sở hữu")
...
async def profile(ctx, member: discord.Member = None):
    ...
    # ĐÃ THÊM MỚI: hiển thị CP (Beat/Verify challenge) và CCP (tạo level) trên hồ sơ.
    cp = u.get("cp", 0)
    ccp = u.get("ccp", 0)
    so_beat = len(u.get("beaten_challenges", []))
    so_verify = len(u.get("verified_challenges", []))
    so_tao = len(u.get("created_challenges", []))
    embed.add_field(name="🏁 CP (Challenge Point)", value=f"{cp} CP — Đã beat {so_beat} | Verify {so_verify} challenge", inline=False)
    embed.add_field(name="🛠️ CCP (Creator Challenge Point)", value=f"{ccp} CCP — Đã tạo {so_tao} challenge", inline=False)
...
async def history(ctx, member: discord.Member = None):
    ...
    for log in logs:
        icon = {"approved": "✅", "rejected": "❌", "removed": "🚫", "edited": "✏️"}.get(log["status"], "•")
