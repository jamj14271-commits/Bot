...
# ĐÃ THÊM MỚI: dùng để xác minh link Youtube làm bằng chứng (!duyet/!nopchallenge) cho
# người máy yếu không quay/chụp được. Lấy key miễn phí tại Google Cloud Console > bật
# "YouTube Data API v3" > tạo API key. KHÔNG bắt buộc - nếu thiếu, bot vẫn chạy bình
# thường nhưng tính năng nộp bằng link Youtube sẽ báo lỗi rõ ràng khi được dùng tới.
YOUTUBE_API_KEY = os.getenv("YOUTUBE_API_KEY")
if not YOUTUBE_API_KEY:
    print("⚠️ CẢNH BÁO: Thiếu biến môi trường YOUTUBE_API_KEY! Tính năng nộp bằng chứng qua link Youtube sẽ không hoạt động.")
...
# ================= XÁC MINH BẰNG CHỨNG QUA LINK YOUTUBE (MỚI) ================= #
...
# Để tránh việc mượn video của người khác nộp khống, bot BẮT BUỘC video phải có tên/ID
# Discord của người nộp trong tiêu đề, mô tả, HOẶC bình luận - nếu không sẽ bị từ chối.
...
async def verify_youtube_evidence(video_id: str, author: discord.Member):
    """Kiểm tra video Youtube có thực sự là của người nộp hay không, bằng cách tìm tên
    hiển thị/username/ID Discord của họ trong tiêu đề, mô tả, hoặc bình luận video.
    Trả về tuple (hop_le: bool, video_url: str|None, ly_do_neu_that_bai: str|None)."""
    if not YOUTUBE_API_KEY:
        return False, None, "Bot chưa được cấu hình YOUTUBE_API_KEY nên chưa hỗ trợ xác minh video Youtube. Vui lòng gửi ảnh/video đính kèm trực tiếp hoặc báo Admin cấu hình."

    # Các chuỗi có thể xuất hiện trong video để chứng minh đây là kênh của người nộp.
    # Bỏ qua chuỗi quá ngắn (<3 ký tự) để tránh so khớp nhầm (VD: tên "A", "Vy").
    candidates = {str(author.id), author.name.lower()}
    if author.display_name: candidates.add(author.display_name.lower())
    if getattr(author, "global_name", None): candidates.add(author.global_name.lower())
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            # 1. Lấy tiêu đề + mô tả video
            url_info = f"https://www.googleapis.com/youtube/v3/videos?part=snippet&id={video_id}&key={YOUTUBE_API_KEY}"
            async with session.get(url_info) as r:
                if r.status != 200:
                    return False, video_url, "Không thể lấy thông tin video từ Youtube (kiểm tra lại link)."
                data = await r.json()
                items = data.get("items", [])
                if not items:
                    return False, video_url, "Không tìm thấy video này (link sai, hoặc video ở chế độ riêng tư/đã xoá)."
                snippet = items[0]["snippet"]
                text_pool = f"{snippet.get('title','')}\n{snippet.get('description','')}".lower()
                # ĐÃ THÊM MỚI: channelId của video, dùng để lọc đúng bình luận của CHỦ KÊNH.
                video_channel_id = snippet.get("channelId")

            found = any(c in text_pool for c in candidates)

            # 2. Nếu chưa thấy ở tiêu đề/mô tả, thử tìm trong bình luận CỦA CHÍNH CHỦ KÊNH
            # (tối đa 100 bình luận đầu). ĐÃ FIX: trước đây quét TẤT CẢ bình luận của mọi
            # người xem, nên ai đó có thể tự bình luận tên/ID Discord của người khác vào
            # video để "xác nhận khống" giúp bạn bè gian lận. Giờ chỉ tính bình luận có
            # authorChannelId TRÙNG với channelId của video (tức đúng người đăng video
            # tự bình luận), các bình luận khác đều bị bỏ qua dù trùng nội dung.
            if not found and video_channel_id:
                try:
                    url_cmt = f"https://www.googleapis.com/youtube/v3/commentThreads?part=snippet&videoId={video_id}&maxResults=100&key={YOUTUBE_API_KEY}"
                    async with session.get(url_cmt) as r2:
                        if r2.status == 200:
                            cdata = await r2.json()
                            for item in cdata.get("items", []):
                                top_comment = item["snippet"]["topLevelComment"]["snippet"]
                                commenter_channel_id = top_comment.get("authorChannelId", {}).get("value")
                                if commenter_channel_id != video_channel_id:
                                    continue  # bỏ qua bình luận không phải của chủ kênh
                                cmt_text = top_comment.get("textDisplay", "").lower()
                                if any(c in cmt_text for c in candidates):
                                    found = True
                                    break
                        # Nếu video tắt bình luận (403 commentsDisabled) thì bỏ qua, không tính là lỗi.
                except Exception:
                    pass

        if found:
            return True, video_url, None
        return False, video_url, "Không tìm thấy tên/ID Discord của bạn trong tiêu đề, mô tả video, hoặc bình luận CỦA CHỦ KÊNH (bình luận của người khác không được tính)."
    except asyncio.TimeoutError:
        return False, video_url, "Yêu cầu kiểm tra video tới Youtube quá thời gian chờ, thử lại sau."
    except Exception as e:
        print(f"[LỖI XÁC MINH YOUTUBE] {e}")
        return False, video_url, "Có lỗi xảy ra khi xác minh video, thử lại sau."
# ================================================================================ #
...
async def check_expiry_task():
    ...
    for loai in ["daily", "event"]:
        ...
        if remaining <= 0 and not data.get("expired_notified"):
            if admin_channel:
                try: await admin_channel.send(f"❌ **{loai.upper()}** (ID: `{data['id']}`) đã hết hạn! Dùng `!thongbao {loai} [ID mới]` để cập nhật mục tiêu mới nhé.")
                except (discord.Forbidden, discord.HTTPException): pass
            if public_channel:
...
class ReviewView(View):
    ...
    async def approve_btn(self, interaction: discord.Interaction, button: Button):
        ...
        # ĐÃ THÊM MỚI (v12): dọn record trong pending_actions vì yêu cầu đã xử lý xong.
        await db.pending_actions.delete_one({"_id": interaction.message.id})
...
async def menu(ctx):
    ...
    embed.add_field(name="💎 1. HỆ THỐNG DUYỆT BÀI & ĐIỂM MP", value="• `!duyet [độ khó/event/daily/danh hiệu]`\n📌 Gắn kèm video/ảnh, hoặc dán **link Youtube** (máy yếu) làm bằng chứng - nhớ ghi tên/ID Discord của bạn ở tiêu đề/mô tả, hoặc tự bình luận trên chính video đó để xác minh chủ kênh!\n• `!bxh` - Xem Bảng Xếp Hạng MP.", inline=False)
    ...
    embed.add_field(name="🏁 Challenge", value="`!nopchallenge [ID level] [tên]` nộp challenge kèm bằng chứng (ảnh/video hoặc link Youtube).\n`!bxhchallenge` xem BXH Challenge.\n`!profile` xem challenge khó nhất bạn đã vượt qua.", inline=True)
...
async def duyet(ctx, *, yeu_cau: str = None):
    ...
    # được video ra tin nhắn) - nếu không có file đính kèm, bot sẽ tìm link Youtube trong
    # nội dung tin nhắn và xác minh đây có đúng là kênh của người nộp hay không.
    ...
    if ctx.message.attachments:
        ...
        msg_check = await ctx.send("🔎 Đang xác minh video Youtube, chờ chút nhé...")
        hop_le, video_url, ly_do = await verify_youtube_evidence(yt_id, ctx.author)
        if not hop_le:
            return await msg_check.edit(content=(
                f"❌ Không thể dùng video này làm bằng chứng: {ly_do}\n"
                f"📌 Hãy ghi **tên hoặc ID Discord của bạn** vào tiêu đề, mô tả, hoặc TỰ BÌNH LUẬN (bằng đúng kênh Youtube của bạn) trên video để chứng minh đây là kênh của bạn, rồi gửi lại lệnh."
            ))
        evidence_url, la_video_link = video_url, True
        try: await msg_check.delete()
    ...
    if la_video_link:
        embed.add_field(name="🔗 Bằng chứng", value=f"📺 [Video Youtube đã xác minh]({evidence_url})", inline=False)
    else:
    ...
    view = ReviewView()
    sent_msg = await admin_channel.send(embed=embed, view=view)
...
class ChallengeRankModal(Modal, title="Duyệt & Xếp hạng Challenge"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await db.challenges.update_one({"_id": self.challenge_id}, {"$set": {"status": "approved", "rank": rank}})
        await log_history(challenge["submitter_id"], "challenge", challenge["name"], "approved", interaction.user.id, f"Xếp hạng #{rank}")
        await log_admin_action(interaction.user.mention, f"Duyệt Challenge #{self.challenge_id}: **{challenge['name']}**", f"<@{challenge['submitter_id']}>", f"Xếp hạng #{rank}")
...
@require_db()
async def nopchallenge(ctx, level_id: str = None, *, ten: str = None):
    ...
    if not level_id or not ten:
        return await ctx.send("❌ VD: `!nopchallenge 12345678 Tên Level/Challenge` (kèm ảnh/video bằng chứng, hoặc dán link Youtube)")

    # ĐÃ THÊM MỚI: cho phép nộp bằng chứng qua link Youtube giống !duyet (xem giải thích
    # chi tiết ở hàm verify_youtube_evidence phía trên).
    evidence_url, la_video_link = None, False
    if ctx.message.attachments:
        evidence_url = ctx.message.attachments[0].url
    else:
...
async def bxhchallenge(ctx, challenge_id: int = None, vi_tri: int = None):
    ...
    if challenge_id is not None and vi_tri is not None:
        ...
        await db.challenges.update_one({"_id": challenge_id}, {"$set": {"rank": vi_tri}})
        await log_admin_action(ctx.author.mention, f"Xếp lại hạng Challenge #{challenge_id}: **{challenge['name']}**", f"<@{challenge['submitter_id']}>", f"Vị trí mới: #{vi_tri}")
        return await ctx.send(f"✅ Đã xếp lại challenge `#{challenge_id}` — **{challenge['name']}** vào vị trí **#{vi_tri}** trên BXH Challenge.")
    ...
    if ranked:
        dong = [f"**#{c['rank']}** — {c['name']} (ID Level `{c.get('level_id','?')}`) — <@{c['submitter_id']}> `#{c['_id']}`" for c in ranked]
        embed.add_field(name="🏆 Đã xếp hạng", value="\n".join(dong)[:1024], inline=False)
    if unranked:
        dong2 = [f"{c['name']} (ID Level `{c.get('level_id','?')}`) — <@{c['submitter_id']}> `#{c['_id']}`" for c in unranked]
    ...
    embed.set_footer(text="Nộp challenge: !nopchallenge [ID level] [tên] | Admin xếp lại hạng: !bxhchallenge [id] [vị trí]")
    await ctx.send(embed=embed)
