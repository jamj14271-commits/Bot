...
async def pick_random_daily_level():
    """Chọn ngẫu nhiên 1 level Insane 8 hoặc 9 sao THẬT. Trả về dict {id,name,difficulty,stars}
    hoặc None nếu GDBrowser lỗi/không tìm được level phù hợp."""
    try:
        timeout = aiohttp.ClientTimeout(total=10)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            url = "https://gdbrowser.com/api/search/*?diff=5&starred=true&count=100"
            async with session.get(url) as r:
                if r.status != 200: return None
                data = await r.json()
                if not isinstance(data, list) or not data: return None
                # Lọc đúng level 8 hoặc 9 sao (API không hỗ trợ lọc sẵn theo số sao)
                candidates = [lv for lv in data if isinstance(lv, dict) and lv.get("stars") in (8, 9) and lv.get("id")]
                if not candidates:
                    candidates = [lv for lv in data if isinstance(lv, dict) and lv.get("id")]
...
async def pick_random_event_level():
    """Chọn ngẫu nhiên 1 level Hard Demon THẬT. Trả về dict {id,name,difficulty,stars}
    hoặc None nếu GDBrowser lỗi/không tìm được level phù hợp."""
    try:
        timeout = aiohttp.ClientTimeout(total=10)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            url = "https://gdbrowser.com/api/search/*?diff=-2&demonFilter=3&starred=true&count=100"
            async with session.get(url) as r:
                if r.status != 200: return None
                data = await r.json()
                if not isinstance(data, list) or not data: return None
...
async def check_expiry_task():
    ...
    for loai in ["daily", "event"]:
        ...
        if not data or not data.get("expires"):
            ...
            picked = await pick_random_daily_level() if loai == "daily" else await pick_random_event_level()
            if picked:
    ...
    async def approve_btn(self, interaction: discord.Interaction, button: Button):
        ...
        if req_type == "mp":
            ...
            if "demon" in item_name and not data.get("is_video_link") and data.get("evidence_url"):
                try:
                    thong_bao_demon = await try_update_hardest_demon(user_id, data["evidence_url"])
                    if thong_bao_demon:
...
async def nopchallenge(ctx, level_id: str = None, *, ten: str = None):
    ...
    if not ten:
        return await ctx.send("❌ Tên challenge không được để trống (sau khi bỏ link Youtube). VD: `!nopchallenge 12345678 Tên Level/Challenge`")
