...
async def check_and_transfer_top1_cp(guild):
    ...
    config = await db.settings.find_one({"_id": "top1_cp_owner"})
    old_top1_id = config["user_id"] if config else None
    ...
    if old_top1_id != new_top1_id:
        ...
        await db.settings.update_one({"_id": "top1_cp_owner"}, {"$set": {"user_id": new_top1_id}}, upsert=True)
...
async def grant_vua_verify_challenge(guild, new_owner_id):
    config = await db.settings.find_one({"_id": "vua_verify_challenge_owner"})
    old_owner_id = config["user_id"] if config else None
    ...
    await db.settings.update_one({"_id": "vua_verify_challenge_owner"}, {"$set": {"user_id": new_owner_id}}, upsert=True)
...
@bot.command(aliases=["capdanhhieu", "grantdanhhieu"])
@commands.has_permissions(administrator=True)
@require_db()
async def adddanhhieu(ctx, member: discord.Member, *, noi_dung: str = None):
    ...
    thong_bao = f"🎖️ Bạn vừa được Admin cấp danh hiệu **{ten_danh_hieu.title()}**!" + (f"\n🏔️ Hardest: **{ten_hardest}**" if ten_hardest else "")
    try:
        await member.send(thong_bao)
    except discord.Forbidden:
        pass
    await ctx.send(f"✅ {chi_tiet} cho {member.mention}.")
...
@bot.command(aliases=["xoadanhhieuadmin", "removedanhhieu"])
@commands.has_permissions(administrator=True)
@require_db()
